              ...
              return (
                <Link key={e.id} href={`/eventos/${e.id}${encontrado ? "?filtro=minhas" : ""}`} className="group block">
                  <div className="relative aspect-[4/5] rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                    ...
                    {e.capa_id
                      ? <img src={`/api/thumb?id=${e.capa_id}&sz=600`} alt={e.nome}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3">
                      <h3 className="text-white font-bold text-sm leading-tight line-clamp-2 mb-1.5">{e.nome}</h3>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1 text-white/70 text-[10px]">
                          <Calendar size={10} />
                          <span>{new Date(e.data).toLocaleDateString("pt-BR", { day: "2-digit", month: "short", year: "numeric" })}</span>
                        </div>
                        {e.total_fotos > 0 && (
                          <span className="text-white/80 text-[10px] font-semibold">{e.total_fotos} fotos</span>
                        )}
                      </div>
                    </div>
                  </div>
